...
label_train <- as.numeric(label_train == "cat")
...
if(run.feature.train){
  tm_rgb_feature_train <- system.time(rgb_feature_train <- feature_rgb(img_train_dir, "train", data_name = "pets", export = F))

}

tm_feature_test <- NA
if(run.feature.test){
  tm_rgb_feature_test <- system.time(rgb_feature_test <- feature_rgb(img_test_dir, "test", data_name = "pets", export = F))
}
...
```{r}
xgb_param <- cv_xgboost(rgb_feature_train, label_train)
```
...
xgb_train <- train_xgboost(rgb_feature, label_train, par = xgb_param)
```
...
pred_test <- test(xgb_train, rgb_feature_test)
sum(pred_test == label_test)
mean(pred_test == label_test)
```
